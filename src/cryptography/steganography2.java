...
class ImageProcess{
    BufferedImage fetchImage() throws Exception{ 
        File f = new File("D:\\cryptography/Flower.jpg");
    ...
    void hideText(BufferedImage img , byte [] txt) throws Exception{
        ...
        switch(in.readLine().trim()){
            case "Y": {
                ...
                Path path = Paths.get("D:\\Decrypt/SteganD.txt");
    ...
    byte[] extractHiddenBytes(BufferedImage img , int size , int offset){
        ...
        for(int l=0;l<size;l++){
            for(int k=0 ; k<8 ; k++){
                Color c = new Color(img.getRGB(j,i)); byte blue = (byte)c.getBlue();
                int red = c.getRed();
                int green = c.getGreen(); 
                int name = +(hiddenBytes[l]<<1);
